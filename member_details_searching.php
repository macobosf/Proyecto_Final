...
{
	 $req = $bdd ->prepare("SELECT * FROM member_entry WHERE firstname LIKE :firstname OR lastname LIKE :lastname OR email LIKE :email OR phone LIKE :phone OR unique_id LIKE :unique_id LIMIT 6") or die("connection failed");
	 $req->execute(array('firstname' => '%'.$_POST['Usersrh'].'%','lastname'=>'%'.$_POST['Usersrh'].'%','email'=>'%'.$_POST['Usersrh'].'%','phone'=>'%'.$_POST['Usersrh'].'%','unique_id'=>'%'.$_POST['Usersrh'].'%'));
	 if($req -> rowCount()==0)
	 {
		?> 
       <div class="line" style="text-align:center; width:379px;"><?PHP echo"<br/>Ningún miembro encontrado!!!"; ?></div>
        <?PHP 
	 }else
		{
		...
		/////
			...
			else
			{
               ...
               <div class="line" style="width:379px;" onclick="javascript:return mem_srch_Detail_show('<?PHP echo $book_count; ?>','<?PHP echo $data['unique_id']; ?>','<?PHP echo $data['dob']; ?>','<?PHP echo ucfirst($data['firstname']); ?>','<?PHP echo ucfirst($data['middlename']); ?>','<?PHP echo ucfirst($data['lastname']); ?>','<?PHP echo $data['gender']; ?>','<?PHP echo $data['email']; ?>','<?PHP echo $data['country']; ?>','<?PHP echo $data['program'];?>','<?PHP echo $data['semester']; ?>','<?PHP echo $data['register_time']; ?>','<?PHP echo $data['phone']; ?>','<?PHP echo $data['district'];?>','<?PHP echo $data['city'];?>','<?PHP echo $img_get;?>')" title="Member ID:<?PHP echo $data['unique_id']; ?>"> <div class="img_style">
...
&nbsp;&nbsp;&nbsp;Número de problema del libro
:&nbsp;<span id="number_color" title="<?PHP for($j=0; $j<$book_count;$j++){ 
...
{
	?>
<div class="line" style="text-align:center; width:379px;"><?PHP echo"<br/>Enter Member's Details !!!"; ?></div>
<?PHP
}
?>